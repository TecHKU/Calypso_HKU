...
class Projects extends Component {
  ...
  onSearch = val => {
    ...
    for (let i = 0; i < this.state.projects.length; i++) {
      if (
        this.state.projects[i].title.toLowerCase().indexOf(val.toLowerCase()) >=
          0 &&
        this.intersectionSet(this.state.roles, this.state.projects[i].roles) &&
        this.intersectionSet(this.state.tags, this.state.projects[i].tags)
      ) {
  ...
  render() {
    return (
      <div className="col-lg-12">
        ...
        <ProjectsScreen
          projects={this.state.displayProjects}
